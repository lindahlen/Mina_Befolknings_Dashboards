...
print("Startar kartgenerering för Val 2026...")
...
# --- FILNAMN FÖR VALKARTAN 2026 ---
GEOJSON_VAL_FILENAME = 'Valkarta2026.geojson'
OUT_HTML_NAME = 'Valdeltagande_2026.html'
...
def clean_for_match(s):
    ...
    return s.upper().replace(' ', '').replace('-', '').split('(')[0].strip()
...
# =====================================================================
# SÄKRA HJÄLPFUNKTIONER (Den STABILA originalmetoden)
# =====================================================================
def get_exact_col(df, possible_names):
    if df is None or df.empty: return None
    c_map = {str(c).strip().upper(): c for c in df.columns}
    for name in possible_names:
        if name.upper() in c_map: return c_map[name.upper()]
    return None

def get_col_contains(df, possible_substrings):
    if df is None or df.empty: return None
    for c in df.columns:
        c_up = str(c).upper()
        if any(sub.upper() in c_up for sub in possible_substrings):
            return c
    return None
...
def get_coord_col(df, is_x):
    ...
    cands = ['x', 'x_koord', 'long', 'longitude'] if is_x else ['y', 'y_koord', 'lat', 'latitude']
    col = next((c_map[c] for c in cands if c in c_map), None)
    if not col:
        for c in df.columns:
            c_low = str(c).strip().lower()
            if is_x and ('long' in c_low or c_low.startswith('x')): return c
            if not is_x and ('lat' in c_low or c_low.startswith('y')): return c
    return col

# RAK OCH STABIL PANDAS MERGE (Hittar ID, rensar dubbletter, klistrar fast)
def simple_left_join(base_df, ext_df, possible_id_cols):
    if ext_df is None or ext_df.empty: return base_df
    
    id_col = get_col_contains(ext_df, possible_id_cols)
    if not id_col: return base_df
    
    ext_df = ext_df.copy()
    ext_df['MATCH_ID'] = ext_df[id_col].apply(normalize_id)
    ext_df = ext_df.dropna(subset=['MATCH_ID']).drop_duplicates(subset=['MATCH_ID'])
    
    overlap = [c for c in ext_df.columns if c in base_df.columns and c != 'MATCH_ID']
    base_df = base_df.drop(columns=overlap, errors='ignore')
    
    return base_df.merge(ext_df, on='MATCH_ID', how='left')

try:
    # =====================================================================
    # 2. LÄS IN GEOGRAFI OCH INFRASTRUKTUR
    # =====================================================================
    print("\nLaddar geografi 2026...")

    valdistrikt_path = os.path.join(kart_filer_dir, GEOJSON_VAL_FILENAME)
    if os.path.exists(valdistrikt_path):
        valdistrikt_gdf = gpd.read_file(valdistrikt_path, encoding='utf-8')
        # Använd EXACT match och prioritera VD_NAMN för att undvika att Valkretsnamnet väljs
        name_col = get_exact_col(valdistrikt_gdf, ['VD_NAMN', 'VDNAMN', 'VALDISTRIKTSNAMN', 'VALDISTRIKT', 'NAMN'])
        if name_col:
            valdistrikt_gdf['NAMN_GEO'] = valdistrikt_gdf[name_col].apply(fix_text)
        else:
            valdistrikt_gdf['NAMN_GEO'] = 'Okänt'
            
        geo_id_col = get_exact_col(valdistrikt_gdf, ['LKFV', 'KOD', 'VALDISTRIKTSKOD', 'ID', 'LÄNKOMMUNKOD'])
        if geo_id_col:
        ...
        valdistrikt_gdf = gpd.GeoDataFrame(columns=['NAMN_GEO', 'geometry', 'MATCH_ID'], geometry='geometry', crs="EPSG:4326")
    ...
    gdf_merged['NAMN'] = gdf_merged['NAMN_GEO'] # Tvinga in kartans namn direkt
    ...
    if df_adress is not None and not df_adress.empty:
        ...
        # Klipp av siffrorna från fastigheten för att få trakt
        ...
        # SNABBFIX FÖR NÅBARHET: Koppla punkter till valdistrikt
    ...
    # --- LÄS IN EXCEL-FLIKAR MED STABIL PANDAS MERGE ---
    ...
    if os.path.exists(valdistrikt_excel_path):
        # LÄSER IN 2026-FLIKAR
        df_vd_2026 = pd.DataFrame()
        df_valdeltagande = pd.DataFrame()
        
        try: df_vd_2026 = pd.read_excel(valdistrikt_excel_path, sheet_name='Valdistrikt2026')
        except Exception: pass
        try: df_valdeltagande = pd.read_excel(valdistrikt_excel_path, sheet_name='Valdeltagande')
        except Exception: pass
        ...
        try: df_hushall = pd.read_excel(valdistrikt_excel_path, sheet_name='Hushåll_2026')
        except Exception: pass
        ...
        if not df_lokaler.empty:
            ...
            if x_col and y_col:
                ...
                if not valid_lok.empty:
                    ...
                    df_lokaler['Lok_Lat'] = np.nan; df_lokaler['Lok_Lon'] = np.nan
                    
                    for idx, row in valid_lok.iterrows():
                        y, x = lok_gdf.loc[idx].geometry.y, lok_gdf.loc[idx].geometry.x
                        if not math.isnan(y) and not math.isnan(x):
                            df_lokaler.loc[idx, 'Lok_Lat'] = y
                            df_lokaler.loc[idx, 'Lok_Lon'] = x
        
        # 2. SAMMANSLAGNING AV EXCEL-FLIKAR (Din stabila originalmetod!)
        if not df_vd_2026.empty:
            gdf_merged = simple_left_join(gdf_merged, df_vd_2026, ['LÄNKOMMUNKOD', 'KODEN', 'LKFV'])
        ...
        if not df_lokaler.empty:
            ...
            if 'VALDISTRIKT' in df_lokaler.columns and namn_col in gdf_merged.columns:
                ...
                gdf_merged = simple_left_join(gdf_merged, df_lokaler, ['LÄNKOMMUNKOD', 'KODEN', 'VALDISTRIKTSKOD', 'LKFV'])
                    
        # --- ROBUST HANTERING AV HUSHÅLL_2026 ---
                ...
                if not valid_poi.empty:
                    ...
                    for idx, row in poi_gdf.iterrows():
                        if pd.notnull(row.geometry.y) and pd.notnull(row.geometry.x):
                            ...
                            # Hämta för 2026 (fungerar även för "fortidsroster" utan prickar)
                            fortidsroster = safe_str(row.get('Antal_förtidsröster_2026', row.get('Antal_fortidsroster_2026', safe_str(row.get('Antal_förtidsröster', '')))))
                            poi_list.append([row.geometry.y, row.geometry.x, namn, funktion, fortidsroster])

    if not agg_rost.empty:
        agg_rost['MATCH_ID'] = agg_rost['MATCH_ID'].astype(str)
        overlap = ['Beraknad_Rostberattigade'] if 'Beraknad_Rostberattigade' in gdf_merged.columns else []
        gdf_merged.drop(columns=overlap, inplace=True, errors='ignore')
        gdf_merged = gdf_merged.merge(agg_rost, on='MATCH_ID', how='left')

    # VIKTIGT: Bygg Vallokaler-listan utifrån vår sammanslagna gdf_merged för att säkra att MATCH_ID är identisk med kartan!
    lokaler_list.clear()
    if not gdf_merged.empty:
        c_map_m = {str(c).strip().lower(): c for c in gdf_merged.columns}
        lok_n = next((c_map_m[c] for c in ['lokal', 'vallokal', 'namn på vallokal'] if c in c_map_m), None)
        adr_n = next((c_map_m[c] for c in ['adress', 'adress1', 'gatuadress'] if c in c_map_m), None)
        for idx, row in gdf_merged.iterrows():
            if pd.notnull(row.get('Lok_Lat')) and pd.notnull(row.get('Lok_Lon')):
                m_id = safe_str(row.get('MATCH_ID', ''))
                dist_namn = safe_str(row.get('NAMN_GEO', ''))
                lokal_name = safe_str(row.get(lok_n, 'Vallokal')) if lok_n else 'Vallokal'
                adress_name = safe_str(row.get(adr_n, '')) if adr_n else ''
                lokaler_list.append([float(row['Lok_Lat']), float(row['Lok_Lon']), lokal_name, adress_name, m_id, dist_namn])
    ...
    partier_abbr = ['M', 'KD', 'L', 'C', 'S', 'V', 'MP', 'SD', 'LL']
    party_mapping = {
        'M': ['M', 'MODERATERNA'],
        'KD': ['KD', 'KRISTDEMOKRATERNA'],
        'L': ['L', 'LIBERALERNA'],
        'C': ['C', 'CENTERPARTIET', 'CENTERN'],
        'S': ['S', 'SOCIALDEMOKRATERNA', 'ARBETAREPARTIET-SOCIALDEMOKRATERNA'],
        'V': ['V', 'VÄNSTERPARTIET'],
        'MP': ['MP', 'MILJÖPARTIET', 'MILJÖPARTIET DE GRÖNA'],
        'SD': ['SD', 'SVERIGEDEMOKRATERNA'],
        'LL': ['LL', 'LINKÖPINGSLISTAN']
    }
    
    party_data_dict_id = {}
    party_data_dict_name = {}
    party_year_global = "2022"
    ...
    if os.path.exists(party_results_path):
        try:
            ...
            sheet_names_upper = {s.strip().upper(): s for s in xls_parti.sheet_names}
            
            for p_abbr in partier_abbr:
                target_sheet = next((sheet_names_upper[alias] for alias in party_mapping[p_abbr] if alias in sheet_names_upper), None)
                if target_sheet:
                    df_p = pd.read_excel(party_results_path, sheet_name=target_sheet)
                    ...
                    p_namn_col = next((c_map_p[c] for c in ['NAMN', 'VALDISTRIKT'] if c in c_map_p), None)
                    
                    val_col = None
                    # Kolla 2026 först, MÅSTE ha siffror över 0!
                    col_2026 = next((c_map_p[c] for c in ['2026', 'VALRESULTAT 2026', 'ANDEL 2026'] if c in c_map_p), None)
                    if col_2026:
                        s_test = pd.to_numeric(df_p[col_2026].astype(str).str.replace(',', '.').str.replace('%', '').str.replace(' ', ''), errors='coerce')
                        if s_test.sum() > 0:
                            val_col = col_2026
                            party_year_global = "2026"
                            
                    # Har 2026 inga siffror? Använd 2022 som tillfälligt fallback-stöd.
                    if not val_col:
                        col_2022 = next((c_map_p[c] for c in ['2022', 'VALRESULTAT 2022', 'ANDEL 2022', 'PROCENT', 'ANDEL', 'RESULTAT', 'RÖSTER (%)'] if c in c_map_p), None)
                        if col_2022:
                            val_col = col_2022
                            party_year_global = "2022"
                    
                    if val_col:
                        if p_id_col: df_p['MATCH_ID'] = df_p[p_id_col].apply(normalize_id)
                        if p_namn_col: df_p['match_namn'] = df_p[p_namn_col].apply(clean_for_match)
                        
                        for _, row_p in df_p.iterrows():
                            val_str = str(row_p[val_col]).replace(',', '.').replace('%', '').replace(' ', '').strip()
                            try:
                                v = float(val_str)
                                # Spara råvärdet. Vi kollar summan senare!
                                if p_id_col and row_p.get('MATCH_ID'):
                                    m_id = str(row_p['MATCH_ID'])
                                    if m_id not in party_data_dict_id: party_data_dict_id[m_id] = {pt: 0.0 for pt in partier_abbr}
                                    party_data_dict_id[m_id][p_abbr] = v
                                    
                                if p_namn_col and row_p.get('match_namn'):
                                    m_namn = str(row_p['match_namn'])
                                    if m_namn not in party_data_dict_name: party_data_dict_name[m_namn] = {pt: 0.0 for pt in partier_abbr}
                                    party_data_dict_name[m_namn][p_abbr] = v
                            except: pass
                            
            # OMÄKNING: Kolla summan av partier per valdistrikt
            # Om summan är <= 1.5 betyder det att siffrorna skrivits i decimaler (0.25). Endast DÅ gångar vi med 100!
            for m_id, p_data in party_data_dict_id.items():
                tot = sum(p_data.values())
                if 0 < tot <= 1.5:
                    for pt in p_data: party_data_dict_id[m_id][pt] = round(p_data[pt] * 100, 1)
                else:
                    for pt in p_data: party_data_dict_id[m_id][pt] = round(p_data[pt], 1)
                    
            for m_namn, p_data in party_data_dict_name.items():
                tot = sum(p_data.values())
                if 0 < tot <= 1.5:
                    for pt in p_data: party_data_dict_name[m_namn][pt] = round(p_data[pt] * 100, 1)
                else:
                    for pt in p_data: party_data_dict_name[m_namn][pt] = round(p_data[pt], 1)

        except Exception as e: print(f"Kunde inte läsa in Partiernas valresultat: {e}")

    # --- KONTROLL: FINNS DET RIKTIG DATA FÖR VALET 2026 ÄNNU? ---
    def check_has_data(df, possible_names):
        if df is None or df.empty: return False
        actual_col = get_col_contains(df, possible_names)
        if actual_col and actual_col in df.columns:
            col_data = df[actual_col]
            if isinstance(col_data, pd.DataFrame): col_data = col_data.iloc[:, 0]
            clean_col = col_data.astype(str).str.replace(' ', '', regex=False).str.replace('\xa0', '', regex=False).str.replace(',', '.', regex=False)
            valid_vals = pd.to_numeric(clean_col, errors='coerce').dropna()
            return len(valid_vals) > 0 and valid_vals.sum() > 0
        return False

    has_valdeltagande = check_has_data(gdf_merged, ['VALDELTAGANDE 2026', 'VALDELTAGANDE %', 'VALDELTAGANDE'])
    has_rostande = check_has_data(gdf_merged, ['RÖSTANDE 2026', 'ANTAL RÖSTANDE', 'RÖSTANDE'])
    has_rostberattigade = check_has_data(gdf_merged, ['RÖSTBERÄTTIGADE 2026', 'ANTAL RÖSTBERÄTTIGADE', 'RÖSTBERÄTTIGADE'])
    has_forsta = check_has_data(gdf_merged, ['ANDEL FÖRSTAGÅNGSVÄLJARE'])
    has_utland = check_has_data(gdf_merged, ['ANDEL UTLÄNDSKA MEDBORGARE'])
    has_ejrost = check_has_data(gdf_merged, ['ANDEL EJ RÖSTANDE'])

    actual_valdelt_col = get_col_contains(gdf_merged, ['VALDELTAGANDE 2026', 'VALDELTAGANDE %', 'VALDELTAGANDE'])
    actual_rostb_col = get_col_contains(gdf_merged, ['RÖSTBERÄTTIGADE 2026', 'ANTAL RÖSTBERÄTTIGADE', 'RÖSTBERÄTTIGADE'])
    actual_rost_col = get_col_contains(gdf_merged, ['RÖSTANDE 2026', 'ANTAL RÖSTANDE', 'RÖSTANDE'])
    actual_forsta_col = get_col_contains(gdf_merged, ['ANDEL FÖRSTAGÅNGSVÄLJARE'])
    actual_utland_col = get_col_contains(gdf_merged, ['ANDEL UTLÄNDSKA MEDBORGARE'])
    actual_ejrost_col = get_col_contains(gdf_merged, ['ANDEL EJ RÖSTANDE'])

    actual_hushall_col = get_col_contains(gdf_merged, ['ANTAL HUSHÅLL', 'HUSHÅLL'])
    actual_hyres_col = get_col_contains(gdf_merged, ['HYRESRÄTT'])
    actual_ensam_col = get_col_contains(gdf_merged, ['ENSAMSTÅENDE'])
    actual_eftergym_col = get_col_contains(gdf_merged, ['EFTERGYMNASIAL'])
    
    has_hushall = True if actual_hushall_col else False

    # --- BYGG DATADICTIONARY FÖR JAVASCRIPT ---
    val_data_dict = {}
    hist_years = ['1998', '2002', '2006', '2010', '2014', '2018', '2022', '2026']

    if not gdf_merged.empty:
        for idx, row in gdf_merged.iterrows():
            match_id = safe_str(row.get('MATCH_ID', 'Okänt'))
            
            # Tvinga namnet att BARA tas från det ursprungliga kartlagret (NAMN_GEO)
            namn = safe_str(row.get('NAMN_GEO', f'Område_{idx}'))
            namn_upper = clean_for_match(namn)
            ...
            valdeltagande = round(safe_num(row.get(actual_valdelt_col, 0)), 1) if actual_valdelt_col else None
            rostberattigade = int(safe_num(row.get(actual_rostb_col, 0))) if actual_rostb_col else None
            rostande = int(safe_num(row.get(actual_rost_col, 0))) if actual_rost_col else None
            forstagangs = round(safe_num(row.get(actual_forsta_col, 0)), 1) if actual_forsta_col else None
            utlandska = round(safe_num(row.get(actual_utland_col, 0)), 1) if actual_utland_col else None
            ej_rostande = round(safe_num(row.get(actual_ejrost_col, 0)), 1) if actual_ejrost_col else None
            
            rostberattigade_2025 = int(safe_num(row.get('Beraknad_Rostberattigade', 0))) 
            
            antal_hushall = int(safe_num(row.get(actual_hushall_col, 0))) if actual_hushall_col else 0
            andel_hyresratt = round(safe_num(row.get(actual_hyres_col, 0)), 1) if actual_hyres_col else 0
            andel_ensamstaende = round(safe_num(row.get(actual_ensam_col, 0)), 1) if actual_ensam_col else 0
            andel_eftergymnasial = round(safe_num(row.get(actual_eftergym_col, 0)), 1) if actual_eftergym_col else 0
            
            delta_valdeltagande = None
            if valdeltagande:
                val_prev = hist_data.get('2022')
                if val_prev is None: val_prev = hist_data.get('2018')
                if val_prev is None: val_prev = safe_num(row.get('2022', row.get('2018', 0)))
                if val_prev > 0: delta_valdeltagande = round(valdeltagande - val_prev, 1)

            lok_lat = row.get('Lok_Lat', None)
            lok_lon = row.get('Lok_Lon', None)
            
            c_map_m = {str(c).strip().lower(): c for c in gdf_merged.columns}
            lokal_col = next((c_map_m[c] for c in ['lokal', 'vallokal', 'namn på vallokal'] if c in c_map_m), None)
            adress_col = next((c_map_m[c] for c in ['adress', 'adress1', 'gatuadress'] if c in c_map_m), None)

            # Hämta in de sanna partivärdena via ID eller Namn
            p_data = {pt: 0.0 for pt in partier_abbr}
            if match_id in party_data_dict_id and any(v > 0 for v in party_data_dict_id[match_id].values()):
                p_data.update(party_data_dict_id[match_id])
            elif namn_upper in party_data_dict_name and any(v > 0 for v in party_data_dict_name[namn_upper].values()):
                p_data.update(party_data_dict_name[namn_upper])
            ...
            val_data_dict[match_id] = {
                ...
                'Vallokal': safe_str(row.get(lokal_col, 'Saknas')) if lokal_col else safe_str(row.get('LOKAL', 'Saknas')),
                'Adress': safe_str(row.get(adress_col, 'Saknas')) if adress_col else safe_str(row.get('ADRESS1', 'Saknas')),
    ...
    # HTML Strängar för dynamisk Döljning om val 2026 ej hållits
    active_val_layers = []
    val_snabbfilter_html = ""
    
    if has_valdeltagande:
        active_val_layers.append("""
        <div class="form-check mb-1">
            <input class="form-check-input var-toggle" type="radio" name="layerToggle" value="Valdeltagande" id="t_valdeltagande">
            <label class="form-check-label" for="t_valdeltagande">🗳️ Valdeltagande (%)</label>
        </div>
        <div class="form-check mb-2" style="background:#f8f9fa; padding-top:2px; padding-bottom:2px; border-radius:4px;">
            ...
            <label class="form-check-label text-danger fw-bold" for="t_delta">📉 Δ Valdeltagande</label>
        </div>""")
        val_snabbfilter_html += '<button id="btn-filter-valdeltagande" class="btn btn-outline-secondary btn-sm btn-custom mb-1" value="VALDELTAGANDE">📉 &lt; 75% Valdeltagande</button>\n'
    ...
    if has_utland:
        ...
        val_snabbfilter_html += '<button id="btn-filter-utland" class="btn btn-outline-secondary btn-sm btn-custom mb-1" value="UTLAND">🌍 &gt; 8% Utländska medborgare</button>\n'
    ...
    hushall_check = ""
    if len(active_val_layers) > 0:
        val_layers_str = "".join(active_val_layers)
        
        if has_valdeltagande:
            val_layers_str = val_layers_str.replace('id="t_valdeltagande"', 'id="t_valdeltagande" checked')
            js_default_var = "Valdeltagande"
        elif has_rostberattigade:
            val_layers_str = val_layers_str.replace('id="t_rostberattigade"', 'id="t_rostberattigade" checked')
            js_default_var = "Rostberattigade"
        else:
            js_default_var = "Rostande"
            val_layers_str = val_layers_str.replace('id="t_rostande"', 'id="t_rostande" checked')

        val_layers_html = f"""
        <hr style="margin: 10px 0;">
        <h6 class="fw-bold mb-2">📊 Valdata Ytor & Analys (2026)</h6>
        {val_layers_str}
        """
    else:
        val_layers_html = """
        <hr style="margin: 10px 0;">
        <h6 class="fw-bold mb-2 text-muted">📊 Valdata Ytor & Analys</h6>
        <div class="alert alert-info py-1 px-2 mb-2" style="font-size:11px; line-height:1.2;">
            <i class="fa-solid fa-hourglass-half"></i> Inväntar valresultat 2026. Fler lager låses upp när data fylls i.
        </div>
        """
        js_default_var = "Antal_hushall" if has_hushall else "Granser"
        hushall_check = "checked" if has_hushall else ""
    ...
    <div class="legend-container" id="legend-container">
        ...
        <div id="legend-Delta_Valdeltagande" class="variable-legend"><h6 style="margin-bottom:5px;font-weight:bold;">Δ Valdeltagande</h6>
    ...
    <div class="layers-panel">
        ...
        <hr style="margin: 10px 0;">
        <h6 class="fw-bold mb-2">🏘️ Hushållsdata (2026)</h6>
        <div class="form-check mb-1">
            <input class="form-check-input var-toggle" type="radio" name="layerToggle" value="Antal_hushall" id="t_hushall" {hushall_check}>
    ...
    <div id="infoPanel" class="info-panel">
        ...
        <ul class="nav nav-tabs mb-3" id="infoTabs" role="tablist">
            <li class="nav-item"><button class="nav-link active" id="overview-tab" data-bs-toggle="tab" data-bs-target="#overview" type="button" role="tab">Översikt</button></li>
        ...
        <div class="tab-content" id="infoTabsContent">
            <div class="tab-pane fade show active" id="overview" role="tabpanel">
                ...
                <div class="mt-3">
                    <p class="mb-1 fw-bold" style="font-size: 12px;" id="partyChartTitle">Partifördelning:</p>
    ...
    <script>
        ...
        var currentActiveVariable = '{js_default_var}';
        ...
        window.showGeneralInfo = function() {{
            showToast("ℹ️ <b>Analys & Verktyg:</b><br><br><b>Snabbfilter:</b> Kombinera Valkrets med t.ex. 'Topp 10' för att direkt se vilka områden som sticker ut mest i den variabel du valt i kartlagren.<br><br><b>Nåbarhet Vallokal:</b> Aktivera verktyget och klicka var som helst i ett valdistrikt för att se gång-, cykel- och bilavstånd till just dess vallokal, samt avståndet till Stadshuset.", 15000);
        }};
        ...
        document.addEventListener('DOMContentLoaded', function() {{
            ...
            function getCleanTooltip(data, variable) {{
                var extraInfo = "";
                var varNames = {{
                    'Delta_Valdeltagande': ['Förändring valdeltagande (22-26):', ' %-enh'],
                ...
                var v2026_label = data.Valdeltagande !== null ? data.Valdeltagande : '?';
                var v2022_label = data.Historik_Valdeltagande['2022'] !== null ? data.Historik_Valdeltagande['2022'] : '?';
                var val_txt = data.Valdeltagande !== null ? `<strong>Valdeltagande (2026):</strong> ${{v2026_label}} %<br>` : `<strong>Valdeltagande (2022):</strong> ${{v2022_label}} % <span style="font-size:11px; color:#e74c3c;">(Inväntar 2026)</span><br>`;
                ...
                return `
                    <div class="val-tooltip" style="font-size: 13px; line-height: 1.4;">
                        ...
                        ${{val_txt}}
                        <strong>Röstberättigade:</strong> ${{data.Rostberattigade !== null ? data.Rostberattigade.toLocaleString('sv-SE') : '?'}} st <span style="font-size:11px; color:#777; font-weight:normal;">(Prel. 2025: ${{data.Rostberattigade_2025.toLocaleString('sv-SE')}} st)</span><br>
                        ${{extraInfo}}
                    </div>
                `;
            }}
            ...
            document.getElementById('searchDistrikt').addEventListener('input', function(e) {{
                ...
                if (data) {{
                    ...
                    var sRost = data.Rostberattigade !== null ? `<b>Röstberättigade:</b> ${{data.Rostberattigade.toLocaleString('sv-SE')}} st<br><span style="font-size:11px; color:#777;">(Prel. 2025: ${{data.Rostberattigade_2025.toLocaleString('sv-SE')}} st)</span><br>` : `<b>Röstberättigade (Prel. 2025):</b> ${{data.Rostberattigade_2025.toLocaleString('sv-SE')}} st<br>`;
            ...
            function showInfoPanel(matchId) {{
                ...
                let htmlContent = `
                    <table class="table table-sm table-borderless mb-2">
                        ...
                        <tr><td><strong>Röstberättigade:</strong></td><td class="text-end">${{data.Rostberattigade !== null ? data.Rostberattigade.toLocaleString('sv-SE') : '?'}} st</td></tr>
                        ...
                        <tr><td><strong>Valdeltagande:</strong></td><td class="text-end fw-bold text-primary">${{data.Valdeltagande !== null ? data.Valdeltagande : '?'}} %</td></tr>
                    </table>
                `;
            ...
            try {{
                ...
                function bindPolygonEvents() {{
                    ...
                    // OM CHECKBOXEN LOKALER ÄR I-KRYSSAD, RITA UT DEM DIREKT
                    if (document.getElementById('toggleLokaler') && document.getElementById('toggleLokaler').checked) {{
                        map.addLayer(lokalerLayerObj);
                    }}
    ...
    print("\n💡 Om du startar Live Server nu, se till att du startar den från 'modermappen' och klickar på 'Valdeltagande_2026.html'!")
