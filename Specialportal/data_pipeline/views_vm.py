    ...
    <script>
        ...
        function renderMatches() {
            ...
            const getPhaseWeight = (phase) => {
                ...
                // Sätt viktning så att Final alltid hamnar före Bronsmatch vid samma datum
                if (p.includes("final") && !p.includes("kvarts") && !p.includes("semi") && !p.includes("åtton") && !p.includes("sexton") && !p.includes("omgång")) return 100;
                if (p.includes("tredje") || p.includes("brons")) return 90;
            ...
            let matchArray = Object.values(db.matches).sort((a, b) => {
                ...
                return getPhaseWeight(b.phase) - getPhaseWeight(a.phase);
            });
        ...
        function renderKnockoutTree(year) {
            ...
            // Ny knivskarp matchningslogik
            koMatches.forEach(m => {
                let p_lower = m.phase.toLowerCase();
                
                if (p_lower.includes("tredje") || p_lower.includes("brons")) {
                    ...
                    // Eftersom vi uteslutit kvartsfinal och semifinal tidigare, vet vi säkert att detta är Finalen
        ...
        function openMatchModal(matchId) {
            ...
            // --- MATCHHÄNDELSER (Mål & Kort med Självmålslogik) ---
            ...
            m.events.goals.forEach(g => {
                ...
                if (pName.toLowerCase() === "självmål") {
                    if (noteText && noteText !== "null" && noteText !== "") {
                        goalText = `⚽ Självmål av ${formatName(noteText)}`;
                    } else {
                        goalText = `⚽ Självmål`;
                    }
                } else {
            ...
            const renderLineup = (teamData, elementId) => {
                ...
                teamData.forEach((p, index) => {
                    if (index === 11) {
                        lHtml += `<li class="py-3 mt-3 mb-1 border-t-2 border-slate-200 text-center text-xs font-black text-slate-400 uppercase tracking-widest bg-slate-50">Avbytare</li>`;
                    }
                    ...
                    if (subStr.includes('in') && subStr.includes('ut')) {
                        subIcon = `<span class="text-green-600 font-bold ml-1 text-[12px]">↑</span><span class="text-red-500 font-bold ml-0.5"text-[12px]">↓</span>`;
        ...
        function renderAdminWarnings() {
            ...
            if (warnings.length > 0) {
                ...
                // Kategorisera varningarna
                let groups = {
                    "🔴 Kritiska Fel & Datafel": [],
                    "🟠 Logikfel (Resultat & Slutspel)": [],
                    "🟡 Saknas i 'Namn' eller 'Trupper'": [],
                    "⚪ Övriga varningar": []
                };
                ...
                for (let [title, list] of Object.entries(groups)) {
                    ...
                    html += `<h3 class="font-bold text-slate-700 mt-6 mb-2 border-b pb-1">${title} <span class="text-xs font-normal text-slate-400">(${list.length})</span></h3>`;
                    html += `<div class="space-y-2 mb-6">`;
                    
                    list.forEach(w => {
                        let c = title.includes("Kritiska") ? "border-red-200 bg-red-50 text-red-800 font-medium" :
                                title.includes("Logikfel") ? "border-orange-200 bg-orange-50 text-orange-800" :
                                title.includes("Saknas") ? "border-yellow-200 bg-yellow-50 text-yellow-800" :
