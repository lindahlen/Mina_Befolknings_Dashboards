...
<body class="bg-slate-50 text-slate-800 font-sans min-h-screen">
    ...
    <header class="bg-indigo-900 text-white shadow-md">
        <div class="max-w-7xl mx-auto px-4 py-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div class="flex flex-col sm:flex-row gap-2">
                <a href="nationella_index.html" class="inline-flex items-center justify-center text-indigo-100 hover:text-white transition-colors text-sm font-medium bg-indigo-800 hover:bg-indigo-700 px-4 py-2 rounded-md shadow-sm border border-indigo-700">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                    Tillbaka till översikten
                </a>
                <a href="Matchanalys_Cupen_Djup_Dashboard.html" class="inline-flex items-center justify-center text-yellow-100 hover:text-white transition-colors text-sm font-medium bg-yellow-600 hover:bg-yellow-500 px-4 py-2 rounded-md shadow-sm border border-yellow-700">
                    🚀 Fördjupning: Målskyttar & Finalfakta
                </a>
            </div>
        </div>
    </header>
    ...
    <main class="max-w-7xl mx-auto px-4 py-8">
        ...
        <section id="tab-bracket" class="tab-content">
            <div class="bg-white p-6 rounded-lg shadow-sm border border-slate-200 mb-6"><div class="flex justify-between items-start mb-4"><div><div class="flex items-center gap-3 mb-2"><span class="text-2xl">🏆</span><h2 class="text-xl font-bold text-slate-800">Cupäventyret (Slutspel)</h2></div></div></div><div class="flex items-end gap-4 max-w-xl"><div class="flex-1"><label class="block text-sm font-medium text-slate-700 mb-1">Säsong</label><div class="flex items-center gap-1"><button onclick="changeSeasonLocal('bracket-season', 1)" class="p-2 bg-slate-100 hover:bg-indigo-100 text-slate-600 hover:text-indigo-700 rounded-md transition-colors border border-slate-200" title="Föregående säsong i listan">◀</button><select id="bracket-season" onchange="renderBracket('tree')" class="w-full border border-slate-300 rounded-md p-2 bg-slate-50 focus:ring-indigo-500"></select><button onclick="changeSeasonLocal('bracket-season', -1)" class="p-2 bg-slate-100 hover:bg-indigo-100 text-slate-600 hover:text-indigo-700 rounded-md transition-colors border border-slate-200" title="Nästa säsong i listan">▶</button></div></div><div class="flex gap-2"><button onclick="renderBracket('list')" class="bg-slate-200 hover:bg-slate-300 text-slate-800 font-medium py-2 px-4 rounded-md shadow-sm transition-colors">Listvy</button><button onclick="renderBracket('tree')" class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-6 rounded-md shadow-sm transition-colors">Trädvy</button></div></div></div>
    ...
    <script>
        ...
        function changeSeasonLocal(selectId, dir) {
            ...
            if (newIdx >= minIdx && newIdx < sel.options.length) {
                ...
                if (selectId === 'bracket-season' && typeof renderBracket === 'function') {
                    renderBracket('tree');
                }
            }
        }
